...
public class Jogador {
    ...
    private static void intercala(int inicio, int meio, int fim) {
        Jogador novoVetor[] = new Jogador[fim - inicio];
        int i = inicio;
        int m = meio;
        int pos = 0;
    ...
    public static void main(String[] args) throws Exception {
        ...
        mergeSort(0, numEntrada);
        tempo = (System.currentTimeMillis() - tempo);

        for (int i = 0; i < numEntrada; i++)
            Jogadores[i].imprimir();

        Arq.openWrite("694370_mergesort.txt");
