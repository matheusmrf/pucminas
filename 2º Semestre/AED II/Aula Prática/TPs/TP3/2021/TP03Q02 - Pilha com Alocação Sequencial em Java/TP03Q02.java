...
class PilhaSequencial {
    ...
    private Musica[] pilhaMusica; // Pode ser de qualquer outro tipo
    private int topo;

    //----------------------------------------------- Metodos especiais -------------------------------------------------------//

    public PilhaSequencial(int tam) {
        this.pilhaMusica = new Musica[tam];
        this.topo = -1;
    }
    ...
    public Musica ultimoValor() {
        // Topo
        if (this.pilhaVazia()) {
            return null;
        }

        return this.pilhaMusica[this.topo];
    }

    // Empilhar algum valor na pilha
    public void empilhar(Musica valor) {
        // Push
        if (this.topo < this.pilhaMusica.length - 1) {
            this.pilhaMusica[++topo] = valor.clone();
        }
    }
    ...
    public void mostrar (){
        ...
        for(int i = 0; i < topo + 1; i++) {
           MyIO.println("[" + i + "] " + pilhaMusica[i].getId() + " ## " + pilhaMusica[i].getArtists() + " ## " + pilhaMusica[i].getNome() + " ## " + pilhaMusica[i].getRealeaseDate() + " ## " + pilhaMusica[i].getAcousticness() + " ## " + pilhaMusica[i].getDanceability() + " ## " + pilhaMusica[i].getInstrumentalness() + " ## " + pilhaMusica[i].getLiveness() + " ## " + pilhaMusica[i].getLoudness() + " ## " + pilhaMusica[i].getSpeechiness() + " ## " + pilhaMusica[i].getEnergy() + " ## " + pilhaMusica[i].getDurationMs());
        }
...
public class TP03Q02 {
    ...
    public static PilhaSequencial searchIdInReturnSequentialStack(Musica[] musica, PilhaSequencial pilha, String[] id, int numEntrada) {
        boolean achou = false;
    ...
    public static PilhaSequencial tratarComandos(String[] entrada, PilhaSequencial lista, Musica[] musica, int tamanho) throws Exception {
        for (int i = 0; i < entrada.length && i < tamanho; i++){
            ...
            switch (linha[0]) {
                case "I":
                    ...
                    lista.empilhar(music);
                break;
                case "R":
                    music = lista.desempilhar();
    ...
    public static void main(String[] args) throws Exception {
        ...
        PilhaSequencial music = new PilhaSequencial(100);

        //Iniciar o relogio
        time.start();

        //Criar as musicas pedidas
        music = searchIdInReturnSequentialStack(m, music, entrada, i);
