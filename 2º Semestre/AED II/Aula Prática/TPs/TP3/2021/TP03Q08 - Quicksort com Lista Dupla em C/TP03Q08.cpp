...
Musica removerDaListaDupla(ListaDupla *lista, int posicao) {
    ...
    if (lista->primeiro == lista->ultimo) {
        ...
        return music;
    } else if (posicao < 0 || posicao > lista->size - 1) {
...
void mostrarListaDupla(ListaDupla *lista) {
    ...
    for (i = lista->primeiro->prox; i != NULL; i = i->prox) {
        ...
        //Tratar a data da Musica
        dateOrganized = i->elemento.realease_date;
        dateOrganized = organizeDate(dateOrganized);
        strcpy(i->elemento.realease_date, dateOrganized);
...
void createLog(clock_t inicio, clock_t fim) {
    ...
    fprintf(arquivo, "Matricula: 699742\tTempo: %lfs\tComparacoes: %d\tMovimentos: %d", resultado, comparisons, moves);
...
void trocarListaDupla(ListaDupla *lista, Musica *music1, Musica *music2) {
    Musica *temp = (Musica*) malloc(sizeof(Musica));
...
void sortByQuickSortDoublyList(ListaDupla *lista, int esq, int dir) {
    int pivo = pesquisarElementoListaDupla(lista, ((esq + dir) / 2))->duration_ms;
    int i = esq, j = dir;
...
int main() {
    ...
    //Ordenar os elementos da lista dupla por meio do quick sort
    sortByQuickSortDoublyList(listadupla, 0, i - 1);

    //Mostrar os elementos na lista dupla
