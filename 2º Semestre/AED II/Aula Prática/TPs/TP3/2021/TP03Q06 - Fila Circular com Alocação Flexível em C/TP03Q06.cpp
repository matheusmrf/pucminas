...
int tamanho(FilaCircularFlexivel *fila){
    int tamanho = 0;
    Celula *i;
...
void mostrar(FilaCircularFlexivel *fila) {
    Celula *i;
    printf("[");
    for (i = fila->primeiro->prox; i != NULL; i = i->prox) {
...
void createLog(clock_t inicio, clock_t fim) {
    FILE* arquivo = fopen("DadosQuestao06.txt", "w");
...
int main() {
    ...
    FilaCircularFlexivel *filaCircularFlexivel;
    filaCircularFlexivel = (FilaCircularFlexivel*)malloc(sizeof(FilaCircularFlexivel)); // Se não dar um malloc não da pra acessar

    //Construtor da fila circular flexivel
    filaCircularFlexivel = new_filaCircularFlexivel(filaCircularFlexivel);

    //Procurar os ids desejados e adicionar na fila circular flexivel
    searchIdAndAddCircularRow(musica, filaCircularFlexivel, entrada, i);
