...
void quatorze ( chars fileName, int x )
{
   ...
   int result = 0;
   int numerador = 1;
   IO_fprintf ( arquivo, "EXEMPLO0714\n");
   IO_fprintf ( arquivo, "NUMERO DIGITADO -> [ %d ]\n", x );
   for ( y = x; y >= 1; y-- )
   {
      result = pow( 3 , y );
...
void quinze ( chars fileName, int n, double x )
{
   ...
   double result = 0;
   int numerador = 1;
   IO_fprintf ( arquivo, "EXEMPLO0715\n");
   IO_fprintf ( arquivo, "quantidade [n]-> [ %d ]\n", n );
   IO_fprintf ( arquivo, "base [x]-> [ %1.0lf ]\n", x );
   for ( int expoente = 0; expoente <= n; expoente++ )
   {
      result = pow( x , expoente );
      IO_fprintf ( arquivo, "%i/%1.2lf\n",numerador ,result );
...
// repetir até desejar parar
   ...
   {
   // identificar
      IO_id ( "Exemplo0715 - Programa - v0.0" );
   // ler do teclado
      IO_println ( "Opcoes" );
      IO_println ( " 0 - parar" );
