...
int mostrarCaracteres (chars palavra, int x)
{
    if (strlen(palavra) > x)
    {
        printf("\n%i: %c", x, palavra[x]);
        mostrarCaracteres (palavra, x + 1);
    }

}
void method15 ( )
{
    int k = IO_readstring("Digite uma cadeia de caracteres ");

    mostrarCaracteres(k, 0);
    IO_pause ( "Apertar ENTER para continuar" );
}
...
// repetir até desejar parar
   ...
   {
   // identificar
      IO_id ( "Exemplo0615 - Programa - v0.0" );
   // ler do teclado
      IO_println ( "Opcoes" );
      IO_println ( " 0 - parar" );
