...
void addIntMatrix ( ref_int_Matrix matrix, ref_int_Matrix other, ref_int_Matrix multiplica )
{
   int x = 0;
   int y = 0;
   int val1, val2;
   for(int i=0; i < matrix->lines; i++)
    {
//        for(int j=0, resultado = 0; j < matrix->columns; j++, resultado=0)
        for(int j=0; j < matrix->columns; j++) // OBS.: EVITAR O USO DE (,)
        {
            resultado = 0;  // OBS.: MELHOR AQUI

            for(int q=0; q < matrix->lines; q++)
            {
                val1 = matrix->data[i][q];
                val2 = other->data[q][j];
                resultado += (val1 * val2);
            }
            multiplica->data[i][j] = resultado;
        }
    }
} // addIntMatrix ( )
...
void method20 ()
{
   ...
   ref_int_Matrix multiplica = NULL;
   IO_id ( "EXEMPLO1020 - Method20 - v0.0" );

   matrix = freadintMatrix ( "RESULTADO18_1.TXT" );
   other  = freadintMatrix ( "RESULTADO18_2.TXT" );
   multiplica = freadintMatrix ( "RESULTADO18_2.TXT" );
   ...
   //multiplica
   IO_printf ( "\n" );
   IO_printf ( " Multiplicacao: \n" );
   addIntMatrix(matrix, other, multiplica);
   printIntMatrix( multiplica );

   IO_printf ( "\n" );

   free_int_Matrix ( matrix );
   free_int_Matrix ( other );
   free_int_Matrix ( multiplica );
...
// repetir até desejar parar
   ...
   // ler do teclado
      IO_println ( "Opcoes" );
      IO_println ( " 0 - parar" );
      IO_println ( " 1 - multiplicar matrizes." );
      IO_println ( "" );
